...
//   Phone, Upload, Eye, Loader2,
// } from "lucide-react";
// import { useToast } from "@/hooks/use-toast";
// import {
//   useAssignTrip,
//   usePreExpenses,
//   useUploadStartDocuments,
//   useDelivery,
//   useActualExpenses,
//   useDriverWage,
//   useSettlement,
...
//   const { mutateAsync, isPending } = useAssignTrip();
...
//         id: trip.id,
//         payload: {
//           carrierType: form.carrier?.toUpperCase() === "EXTERNAL" ? "EXTERNAL" : "INTERNAL",
//           businessUnit: form.businessUnit,
//           truckId:   form.truckNumber,
//           driverId:  form.driver,
//           internalTripNumber: form.internalTripNumber,
//           assignmentInvoice: form.invoiceNumber,
//           tripPrice: Number(form.tripPriceMXN),
//         },
//       });
//       onUpdate(form);
//       toast({ title: "Load assignment saved ✓" });
//     } catch (err: any) {
//       toast({
...
//     updated[index] = { ...updated[index], [key]: key === "amount" ? Number(value) : value };
//     setForm((prev) => ({ ...prev, otherExpenses: updated }));
//   };
...
//   const otherTotal = form.otherExpenses.reduce((sum: number, e: any) => sum + (e.amount || 0), 0);
//   const total =
...
//     (form.driverFood || 0) +
//     otherTotal;
//   const distance = trip.planning?.tripDistance || 0;
...
//         id: trip.id,
//         payload: {
//           estimatedDiesel: form.estimatedDiesel || 0,
//           estimatedDef: form.estimatedDEF || 0,
//           estimatedCashTolls: form.cashTolls || 0,
//           estimatedDriverFood: form.driverFood || 0,
//           estimatedOther: otherTotal,
//           notes: form.otherExpenses
//             .map((e: any) => `${e.description}: $${e.amount}`)
//             .join(", ") || "",
//         },
//       });
//       onUpdate(form);
//       toast({ title: "Estimated costs saved ✓" });
//     } catch (err: any) {
//       toast({
...
//           Trip distance: <strong>{distance} km</strong>. Diesel auto-estimated at distance ÷ 1.8 =
//           <strong> {autoDiesel} L</strong>.
//         </div>

//         <FormGrid cols={2}>
//           <Field label="Diesel (liters)">
//             <Input type="number" value={form.estimatedDiesel} onChange={(e) => set("estimatedDiesel", e.target.value)} />
//           </Field>
//           <Field label="DEF (MXN)">
//             <Input type="number" value={form.estimatedDEF} onChange={(e) => set("estimatedDEF", e.target.value)} />
//           </Field>
//           <Field label="Cash Tolls (MXN)">
//             <Input type="number" value={form.cashTolls} onChange={(e) => set("cashTolls", e.target.value)} />
//           </Field>
//           <Field label="Driver Food (MXN)">
//             <Input type="number" value={form.driverFood} onChange={(e) => set("driverFood", e.target.value)} />
//           </Field>
//         </FormGrid>

//         <div className="mt-6">
//           <div className="flex justify-between items-center mb-3">
//             <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Other Expected Expenses</p>
//             <Button size="sm" variant="outline" onClick={addExpense}>Add Charge</Button>
//           </div>
...
//                 />
//                 <Button variant="ghost" size="sm" className="col-span-2 text-red-500" onClick={() => removeExpense(index)}>
//                   Remove
//                 </Button>
//               </div>
//             ))}
...
//           <p className="text-sm font-semibold text-muted-foreground">Total Estimated Cost</p>
//           <p className="text-xl font-bold text-foreground">${total.toLocaleString()} MXN</p>
//         </div>
...
// // Admin: read-only view + fallback document upload
// // POST /documents/trip/start-documents
// // ─────────────────────────────────────────────────────────────────────────────
...
// }) {
//   const { toast } = useToast();
//   const { mutateAsync: uploadDocs, isPending: uploading } = useUploadStartDocuments();
...
//             <AdminUploadCard
//               label="Load Acceptance Document"
//               accept="image/*,application/pdf"
//               onUpload={async (files) => {
//                 const fd = new FormData();
//                 files.forEach((f) => fd.append("loadAcceptanceDocument", f));
...
//               onUpload={async (files) => {
//                 const fd = new FormData();
//                 files.forEach((f) => fd.append("tripStartPhoto", f));
//                 fd.append("tripId", trip.id);
//                 try {
//                   await uploadDocs(fd);
...
//                   entry.type === "sleep" ? "bg-indigo-50 border-indigo-200" : "bg-emerald-50 border-emerald-200"
//                 }`}
//               >
...
// // Admin: read-only + fallback container photo upload
// // POST /documents/trip/start-documents  (reuses same endpoint with container photos)
// // ─────────────────────────────────────────────────────────────────────────────
...
// }) {
//   const { toast } = useToast();
//   const { mutateAsync: uploadDocs } = useUploadStartDocuments();
...
//                 Container {i + 1}
//                 {container.customer && (
//                   <span className="ml-2 normal-case font-normal text-muted-foreground">— {container.customer}</span>
//                 )}
//               </p>
...
//                     onUpload={async (files) => {
//                       const fd = new FormData();
//                       files.forEach((f) => fd.append(`container${i + 1}Photos`, f));
//                       fd.append("tripId", trip.id);
//                       try {
//                         await uploadDocs(fd);
//                         toast({ title: `Container ${i + 1} photos uploaded ✓` });
//                       } catch {
//                         toast({ title: "Upload failed", variant: "destructive" });
//                       }
//                     }}
...
// // ─────────────────────────────────────────────────────────────────────────────
// // M6 – Monitoring In Transit  (read-only, no mutations)
// // ─────────────────────────────────────────────────────────────────────────────
...
// // PATCH /trips/:id/delivery  (only when trip is IN_TRANSIT or REACHED)
// // ─────────────────────────────────────────────────────────────────────────────
// export function UnloadingTab({ trip, onUpdate }: { trip: Trip; onUpdate: (d: any) => void }) {
//   const { toast } = useToast();
//   const { mutateAsync, isPending } = useDelivery();
//   const [form, setForm] = useState(trip.unloading);
//   const set = (k: string, v: any) => setForm((p) => ({ ...p, [k]: v }));

//   const canSubmit = ["in_transit", "reached", "IN_TRANSIT", "REACHED"].includes(trip.status);

//   const handleSave = async () => {
//     if (!canSubmit) {
//       toast({
//         title: "Trip not in transit",
//         description: "Delivery can only be recorded when the trip is IN_TRANSIT or REACHED.",
//         variant: "destructive",
//       });
//       return;
//     }
//     try {
//       // Build per-container delivery payload from unloading form containers
//       const containers = (trip.containers ?? []).map((_: any, containerIndex: number) => ({
//         containerIndex,
//         actualDeliveryDate: form.deliveryEndDate
//           ? new Date(form.deliveryEndDate).toISOString()
//           : new Date().toISOString(),
//         actualDeliveryHour: form.deliveryEndHour || "00:00",
//         podPhotos: (form.podPhotos ?? []).map((url: string) => ({ url })),
//       }));
...
//         payload: {
//           containers,
//           driverEndTrip: form.tripEndedByDriver ?? false,
//         },
//       });
//       onUpdate(form);
//       toast({ title: "POD data saved ✓" });
//     } catch (err: any) {
//       toast({
//         title: "Failed to save POD",
//         description: err?.response?.data?.message,
//         variant: "destructive",
//       });
//     }
...
//           ⚠️ Delivery data can only be submitted when the trip status is <strong>IN_TRANSIT</strong> or <strong>REACHED</strong>.
//         </div>
//       )}
...
//           <Textarea placeholder="Any delivery notes…" value={form.notes} onChange={(e) => set("notes", e.target.value)} />
//         </div>

//         <div className="flex justify-end mt-4">
//           <Button onClick={handleSave} disabled={isPending || !canSubmit} className="gap-2">
//             {isPending && <Loader2 className="h-4 w-4 animate-spin" />}
//             Save POD Data
//           </Button>
//         </div>
...
//         <div className="text-sm text-muted-foreground mb-4">
//           The driver can end their portion of the trip. The container lifecycle continues — additional transport may
//           be arranged via local truck or 3rd party, with costs recorded in Extra Charges.
//         </div>
...
// // PATCH /trips/:id/container-return
// // ─────────────────────────────────────────────────────────────────────────────
// export function ContainerDeliveryTab({ trip, onUpdate }: { trip: Trip; onUpdate: (d: any) => void }) {
//   const { toast } = useToast();
//   // ContainerReturn hook is imported inside ExtraChargesTab; re-import here directly
//   const { mutateAsync, isPending } = useContainerReturnHook();
...
//       const containers = (trip.containers ?? []).map((_: any, containerIndex: number) => ({
//         containerIndex,
//         deadFreight: form.deadFreight ?? false,
//         lastFreeDay: form.lastFreeDay
//           ? new Date(form.lastFreeDay).toISOString()
//           : new Date().toISOString(),
//         emptyReturnDate: form.containerDeliveryDate || "",
//         emptyReturnHour: form.containerDeliveryHour || "",
//         portTerminalName: form.portTerminalName || "",
//         portTerminalLocation: form.portTerminalLocation
//           ? { latitude: 0, longitude: 0, address: form.portTerminalLocation }
//           : undefined,
//         emptyReturnCarrier: form.carrier || "",
//         emptyReturnTruckNumber: form.truckNumber || "",
//       }));
...
//         payload: {
//           containers,
//           companyTripEndDate: form.tripEndDate || "",
//           companyTripEndHour: form.tripEndHour || "",
//         },
//       });
//       onUpdate(form);
//       toast({ title: "Container return saved ✓ Trip is now CLOSED." });
//     } catch (err: any) {
//       toast({
//         title: "Failed to save container return",
//         description: err?.response?.data?.message,
//         variant: "destructive",
//       });
//     }
...
// // PATCH /trips/:id/expenses/actual  (only after trip is delivered or completed)
// // ─────────────────────────────────────────────────────────────────────────────
// export function TripExpenseTab({ trip, onUpdate }: { trip: Trip; onUpdate: (d: any) => void }) {
//   const { toast } = useToast();
//   const { mutateAsync, isPending } = useActualExpenses();
//   const [form, setForm] = useState(trip.tripExpense);
//   const est = trip.expensePreAssignment;
//   const set = (k: string, v: string) => setForm((p) => ({ ...p, [k]: Number(v) }));

//   const canSubmit = ["delivered", "completed", "closed", "DELIVERED", "COMPLETED", "CLOSED"].includes(trip.status);

//   const otherExpensesTotal = est.otherExpenses?.reduce((sum: number, e: any) => sum + Number(e.amount || 0), 0) || 0;
//   const estimatedTotal =
//     (est.estimatedDiesel || 0) + (est.estimatedDEF || 0) + (est.cashTolls || 0) + (est.driverFood || 0) + otherExpensesTotal;
...
//     if (!canSubmit) {
//       toast({
//         title: "Trip not yet delivered",
//         description: "Actual expenses can only be recorded after the trip is delivered or completed.",
//         variant: "destructive",
//       });
//       return;
//     }
...
//         id: trip.id,
//         payload: {
//           realDiesel: form.diesel || 0,
//           realDef: form.def || 0,
//           realCashTolls: form.tolls || 0,
//           realDriverFood: form.food || 0,
//           additionalDriverExpenses: form.additionalDriverExpenses || 0,
//           additionalExpenseNotes: form.additionalExpenseNotes || "",
//         },
//       });
//       onUpdate(form);
//       toast({ title: "Trip expenses saved ✓" });
//     } catch (err: any) {
//       toast({
...
//       {!canSubmit && (
//         <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-sm text-amber-800">
//           ⚠️ Actual expenses can only be recorded after the trip is <strong>DELIVERED</strong> or <strong>COMPLETED</strong>.
//         </div>
//       )}
...
//               <CostRow label="Additional Driver Expenses" estimated={otherExpensesTotal} actual={form.additionalDriverExpenses} />
//             </tbody>
//           </table>
//         </div>
...
// // ─────────────────────────────────────────────────────────────────────────────
// // M12 – Driver Wages
// // PATCH /trips/:id/driver-wage
// // ─────────────────────────────────────────────────────────────────────────────
// export function DriverWagesTab({ trip, onUpdate }: { trip: Trip; onUpdate: (d: any) => void }) {
//   const { toast } = useToast();
//   const { mutateAsync, isPending } = useDriverWage();
//   const [form, setForm] = useState(trip.driverWages);
//   const set = (k: string, v: string) => setForm((p) => ({ ...p, [k]: Number(v) }));

//   const salePrice = trip.planning?.tripPrice || 0;
//   const finalWage =
//     (form.baseWage || 0) -
//     (form.imss || 0) -
//     (form.infonavit || 0) -
//     (form.taxes || 0) -
//     (form.driverExtraExpenses || 0);
...
//         id: trip.id,
//         payload: {
//           baseWageOverride: form.baseWage || 0,
//           imssDeduction: form.imss || 0,
//           infonavitDeduction: form.infonavit || 0,
//           taxDeductions: form.taxes || 0,
//           driverExtraExpenses: form.driverExtraExpenses || 0,
//         },
//       });
//       onUpdate({ ...form, finalWage });
//       toast({ title: "Driver wages saved ✓" });
//     } catch (err: any) {
//       toast({
//         title: "Failed to save wages",
//         description: err?.response?.data?.message,
//         variant: "destructive",
//       });
//     }
...
//         <div className="text-sm text-muted-foreground mb-4">
//           Base wage ≈ 9% of sale price. Trip price:{" "}
//           <strong>${salePrice.toLocaleString()} MXN</strong> → Suggested:{" "}
//           <strong>${Math.round(salePrice * 0.09).toLocaleString()} MXN</strong>.
//         </div>
...
//           <MetricCard
//             label="Deductions"
//             value={`-$${((form.imss || 0) + (form.infonavit || 0) + (form.taxes || 0) + (form.driverExtraExpenses || 0)).toLocaleString()}`}
//             color="red"
//           />
...
//     (exp.additionalDriverExpenses || 0) +
//     (addCosts?.roadServiceCost || 0) +
//     (addCosts?.localFreightFee || 0);
...
//   const netProfit = (trip.planning?.tripPrice || 0) + extraRevenue - totalTripCosts - totalDriverCosts;
...
//         id: trip.id,
//         payload: {
//           taxRate: Number(form.taxApplied) / 100 || 0.16,
//           creditTermsDays: Number(form.creditTermsDays) || 30,
//           dueDate: form.dueDate ? new Date(form.dueDate).toISOString() : undefined,
//         },
//       });
//       onUpdate({ ...form, netProfit, totalTripCosts, totalDriverCosts });
//       toast({ title: "Settlement saved ✓" });
//     } catch (err: any) {
//       toast({
//         title: "Failed to save settlement",
//         description: err?.response?.data?.message,
//         variant: "destructive",
//       });
//     }
...
//           <MetricCard label="Trip Price" value={`$${(trip.planning?.tripPrice || 0).toLocaleString()}`} color="blue" />
...
//                 <td className="py-3 px-4 text-right text-emerald-700 font-bold">${(trip.planning?.tripPrice || 0).toLocaleString()}</td>
//               </tr>
...
//           ) : (
//             <p className="text-xs text-blue-400">{uploading ? "Uploading…" : "Click to upload"}</p>
//           )}
//         </div>
//       </label>
//     </div>
...
// // Local alias so ContainerDeliveryTab can use the hook without a circular import
// function useContainerReturnHook() {
//   const { useContainerReturn } = require("@/hooks/useTrips");
//   return useContainerReturn();
// }
...
const defaultExpensePreAssignment = () => ({
  estimatedDiesel: 0,
  estimatedDEF: 0,
  cashTolls: 0,
  driverFood: 0,
  otherExpenses: [] as { description: string; amount: number }[],
});

const defaultDriverAcceptance = () => ({
  loadAcceptanceDate: "",
  loadAcceptanceHour: "",
  tripStartDate: "",
  tripStartHour: "",
  sleepBreakLog: [] as any[],
});

const defaultLoadingData = (trip: Trip) => ({
  arrivalDate: "",
  arrivalHour: "",
  containers: Array.isArray(trip.containers)
...
const defaultUnloading = () => ({
  deliveryEndDate: "",
  deliveryEndHour: "",
  podPhotos: [] as string[],
  notes: "",
  tripEndedByDriver: false,
  driverTripEndDate: "",
  driverTripEndHour: "",
});

const defaultContainerDelivery = () => ({
  deadFreight: false,
  lastFreeDay: "",
...
const defaultTripExpense = () => ({
  diesel: 0,
  def: 0,
  tolls: 0,
  food: 0,
  additionalDriverExpenses: 0,
  additionalExpenseNotes: "",
});

const defaultDriverWages = () => ({
  baseWage: 0,
  imss: 0,
  infonavit: 0,
  taxes: 0,
  driverExtraExpenses: 0,
  finalWage: 0,
});
...
//   const { toast } = useToast();
//   const { mutateAsync, isPending } = useAssignTrip();
//   const [form, setForm] = useState({ ...defaultLoadAssignment(), ...trip.loadAssignment });
//   const set = (k: string, v: string) => setForm((p) => ({ ...p, [k]: v }));
...
//           </Field>
//           <Field label="Carrier Type">
//             <Select value={form.carrier ?? "Internal"} onValueChange={(v) => set("carrier", v)}>
//               <SelectTrigger><SelectValue /></SelectTrigger>
//               <SelectContent>
//                 <SelectItem value="Internal">Internal</SelectItem>
//                 <SelectItem value="External">External</SelectItem>
...
//           </Field>
//           <Field label="Trip Price (MXN)">
//             <Input type="number" value={form.tripPriceMXN ?? 0} onChange={(e) => set("tripPriceMXN", e.target.value)} />
//           </Field>
//           <Field label="Internal Trip Number">
//             <Input value={form.internalTripNumber ?? ""} onChange={(e) => set("internalTripNumber", e.target.value)} />
//           </Field>
...
export function LoadAssignmentTab({
  ...
  const [form, setForm] = useState({
    ...
    // ensure IDs exist
    truckId: trip.loadAssignment?.truckId || "",
    driverId: trip.loadAssignment?.driverId || "",
  });
  ...
  const handleSave = async () => {
    try {
      await mutateAsync({
        ...
        payload: {
          carrierType:
            form.carrier?.toUpperCase() === "EXTERNAL"
              ? "EXTERNAL"
              : "INTERNAL",
      ...
      <ModuleSection title="Load Assignment" icon={<Truck className="h-4 w-4" />}>
        ...
        <FormGrid cols={2}>
          ...
          <Field label="Trip Price (MXN)">
            <Input
              type="number"
              value={form.tripPriceMXN ?? 0}
              onChange={(e) => set("tripPriceMXN", e.target.value)}
            />
          </Field>
          ...
          <Field label="Internal Trip Number">
            <Input
              value={form.internalTripNumber ?? ""}
              onChange={(e) => set("internalTripNumber", e.target.value)}
            />
          </Field>
          ...
          <Field label="Date of Assignment">
            <Input
              ...
              onChange={(e) => set("dateOfAssignment", e.target.value)}
            />
          </Field>
        </FormGrid>
        ...
        {form.carrier === "External" && (
          <div className="mt-4 p-4 rounded-xl bg-amber-50 border border-amber-200 text-sm text-amber-800">
            ⚠️ This trip will be marked as <strong>OUTSOURCED</strong>.
          </div>
        )}
...
export function ExpensePreAssignmentTab({ trip, onUpdate }: { trip: Trip; onUpdate: (d: any) => void }) {
  ...
  const otherTotal = form.otherExpenses.reduce((sum: number, e: any) => sum + (Number(e.amount) || 0), 0);
  const total =
    (form.estimatedDiesel || 0) +
    (form.estimatedDEF || 0) +
    (form.cashTolls || 0) +
    ...
    otherTotal;
  const distance = trip.planning?.tripDistance || 0;
  const autoDiesel = distance ? (distance / 1.8).toFixed(0) : "0";
  ...
  return (
    <div className="space-y-6">
      <ModuleSection title="Estimated Trip Costs" icon={<DollarSign className="h-4 w-4" />}>
        ...
        <FormGrid cols={2}>
          <Field label="Diesel (liters)">
            <Input type="number" value={form.estimatedDiesel ?? 0} onChange={(e) => set("estimatedDiesel", e.target.value)} />
          </Field>
          <Field label="DEF (MXN)">
            <Input type="number" value={form.estimatedDEF ?? 0} onChange={(e) => set("estimatedDEF", e.target.value)} />
          </Field>
          <Field label="Cash Tolls (MXN)">
            <Input type="number" value={form.cashTolls ?? 0} onChange={(e) => set("cashTolls", e.target.value)} />
          </Field>
          <Field label="Driver Food (MXN)">
            <Input type="number" value={form.driverFood ?? 0} onChange={(e) => set("driverFood", e.target.value)} />
          </Field>
        </FormGrid>
...
export function DriverAcceptanceTab({
  ...
  const { toast } = useToast();
  const { mutateAsync: uploadDocs } = useUploadStartDocuments();
  const [form, setForm] = useState({ ...defaultDriverAcceptance(), ...trip.driverAcceptance });
  const set = (k: string, v: any) => setForm((p) => ({ ...p, [k]: v }));
...
export function LoadingDataTab({
  ...
  const { mutateAsync: uploadDocs } = useUploadStartDocuments();
  const [form, setForm] = useState({
    ...defaultLoadingData(trip),
    ...trip.loadingData,
    containers: trip.loadingData?.containers?.length
      ? trip.loadingData.containers
      : defaultLoadingData(trip).containers,
  });
  const set = (k: string, v: string) => setForm((p) => ({ ...p, [k]: v }));
  const setContainer = (i: number, k: string, v: string) => {
    setForm((p) => {
...
export function UnloadingTab({ trip, onUpdate }: { trip: Trip; onUpdate: (d: any) => void }) {
  const { toast } = useToast();
  const { mutateAsync, isPending } = useDelivery();
  const [form, setForm] = useState({ ...defaultUnloading(), ...trip.unloading });
  const set = (k: string, v: any) => setForm((p) => ({ ...p, [k]: v }));
  ...
  const handleSave = async () => {
    ...
    try {
      const tripContainers = Array.isArray(trip.containers)
        ? trip.containers
        : Array.from({ length: Number(trip.containers) || 1 });

      const containers = tripContainers.map((_: any, containerIndex: number) => ({
        containerIndex,
        actualDeliveryDate: form.deliveryEndDate
          ? new Date(form.deliveryEndDate).toISOString()
          : new Date().toISOString(),
...
export function ContainerDeliveryTab({ trip, onUpdate }: { trip: Trip; onUpdate: (d: any) => void }) {
  const { toast } = useToast();
  const { mutateAsync, isPending } = useContainerReturnHook();
  const [form, setForm] = useState({ ...defaultContainerDelivery(), ...trip.containerDelivery });
  const set = (k: string, v: any) => setForm((p) => ({ ...p, [k]: v }));
...
export function TripExpenseTab({ trip, onUpdate }: { trip: Trip; onUpdate: (d: any) => void }) {
  const { toast } = useToast();
  const { mutateAsync, isPending } = useActualExpenses();
  const [form, setForm] = useState({ ...defaultTripExpense(), ...trip.tripExpense });
  const est = { ...defaultExpensePreAssignment(), ...trip.expensePreAssignment };
  const set = (k: string, v: string) => setForm((p) => ({ ...p, [k]: Number(v) }));

  const canSubmit = ["delivered", "completed", "closed", "DELIVERED", "COMPLETED", "CLOSED"].includes(trip.status);

  const otherExpensesTotal = (est.otherExpenses || []).reduce((sum: number, e: any) => sum + Number(e.amount || 0), 0);
  const estimatedTotal =
    (est.estimatedDiesel || 0) + (est.estimatedDEF || 0) + (est.cashTolls || 0) + (est.driverFood || 0) + otherExpensesTotal;
  const actualTotal =
    (form.diesel || 0) + (form.def || 0) + (form.tolls || 0) + (form.food || 0) + (form.additionalDriverExpenses || 0);
  const variance = actualTotal - estimatedTotal;
  ...
  return (
    <div className="space-y-6">
      ...
      <ModuleSection title="Trip Expense Reconciliation" icon={<ReceiptText className="h-4 w-4" />}>
        ...
        <FormGrid cols={2}>
          <Field label="Actual Diesel (MXN)">
            <Input type="number" value={form.diesel ?? 0} onChange={(e) => set("diesel", e.target.value)} />
          </Field>
          <Field label="Actual DEF (MXN)">
            <Input type="number" value={form.def ?? 0} onChange={(e) => set("def", e.target.value)} />
          </Field>
          <Field label="Actual Tolls (MXN)">
            <Input type="number" value={form.tolls ?? 0} onChange={(e) => set("tolls", e.target.value)} />
          </Field>
          <Field label="Actual Food (MXN)">
            <Input type="number" value={form.food ?? 0} onChange={(e) => set("food", e.target.value)} />
          </Field>
          <Field label="Additional Driver Expenses (MXN)" span>
            <Input type="number" value={form.additionalDriverExpenses ?? 0} onChange={(e) => set("additionalDriverExpenses", e.target.value)} />
          </Field>
        </FormGrid>
...
export function DriverWagesTab({ trip, onUpdate }: { trip: Trip; onUpdate: (d: any) => void }) {
  const { toast } = useToast();
  const { mutateAsync, isPending } = useDriverWage();
  const [form, setForm] = useState({ ...defaultDriverWages(), ...trip.driverWages });
  const set = (k: string, v: string) => setForm((p) => ({ ...p, [k]: Number(v) }));
  ...
  return (
    <div className="space-y-6">
      <ModuleSection title="Driver Wages" icon={<BadgeDollarSign className="h-4 w-4" />}>
        ...
        <FormGrid cols={2}>
          <Field label="Base Wage (MXN)">
            <Input type="number" value={form.baseWage ?? 0} onChange={(e) => set("baseWage", e.target.value)} />
          </Field>
          <Field label="IMSS – Social Security (MXN)">
            <Input type="number" value={form.imss ?? 0} onChange={(e) => set("imss", e.target.value)} />
          </Field>
          <Field label="Infonavit – Housing Credit (MXN)">
            <Input type="number" value={form.infonavit ?? 0} onChange={(e) => set("infonavit", e.target.value)} />
          </Field>
          <Field label="Income Taxes (MXN)">
            <Input type="number" value={form.taxes ?? 0} onChange={(e) => set("taxes", e.target.value)} />
          </Field>
          <Field label="Driver Extra Expenses (MXN)" span>
            <Input type="number" value={form.driverExtraExpenses ?? 0} onChange={(e) => set("driverExtraExpenses", e.target.value)} />
          </Field>
        </FormGrid>
...
export function SettlementTab({ trip, onUpdate }: { trip: Trip; onUpdate: (d: any) => void }) {
  const { toast } = useToast();
  const { mutateAsync, isPending } = useSettlement();
  const [form, setForm] = useState({ ...defaultSettlement(), ...trip.settlement });
  const set = (k: string, v: any) => setForm((p) => ({ ...p, [k]: v }));

  const exp = { ...defaultTripExpense(), ...trip.tripExpense };
  const wages = { ...defaultDriverWages(), ...trip.driverWages };
  const extraCharges = (trip as any).extraCharges ?? {};
  const addCosts = { ...trip.additionalCosts };
  ...
  return (
    <div className="space-y-6">
      ...
      <ModuleSection title="Invoicing" icon={<ReceiptText className="h-4 w-4" />}>
        <FormGrid cols={2}>
          <Field label="Tax Rate (%)">
            <Input type="number" value={form.taxApplied ?? 16} onChange={(e) => set("taxApplied", e.target.value)} />
          </Field>
          <Field label="Credit Terms (days)">
            <Input type="number" value={form.creditTermsDays ?? 30} onChange={(e) => set("creditTermsDays", e.target.value)} />
          </Field>
          <Field label="Due Date">
            <Input type="date" value={form.dueDate ?? ""} onChange={(e) => set("dueDate", e.target.value)} />
          </Field>
          <ReadonlyField label="Payment Reminders Sent" value={form.paymentRemindersSent ?? 0} />
        </FormGrid>
